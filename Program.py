...
def process_image(index, threshold_value):
    ...
    concatenated_image = np.concatenate(
        (original, cv2.cvtColor(clahe_img, cv2.COLOR_GRAY2BGR), threshold_img), axis=1
    )
...
process_image(index, threshold_value)  # 처음 이미지 처리 및 출력
...
while True:
    ...
    if key == ord("."):  # 'n' 키가 눌렸을 때 다음 이미지로 이동
        ...
        if index >= len(file_list):  # 마지막 이미지에서 'n' 키를 누르면 프로그램 종료
            break
        process_image(index, threshold_value)
