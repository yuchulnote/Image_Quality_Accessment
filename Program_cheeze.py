...
def remove_large_components(image):
...
def concatenate_images(original, clahe_img, threshold_img):
    clahe_img_bgr = cv2.cvtColor(clahe_img, cv2.COLOR_GRAY2BGR)
    return np.concatenate((original, clahe_img_bgr, threshold_img), axis=1)


def add_image_info(image, filename, threshold_value):
    empty_space = np.zeros((50, image.shape[1], 3), dtype=np.uint8)
    cv2.putText(
        empty_space,
        f"Filename: {filename}",
        (10, 20),
...
def process_image(index, threshold_value):
    ...
    threshold_img = remove_large_components(threshold_img)
    final_image = concatenate_images(original, clahe_img, threshold_img)
    final_image = add_image_info(final_image, filename, threshold_value)
    return final_image
...
while True:
    key = cv2.waitKey(0)
    if key == ord("."):  # If 'n' is pressed, move to the next image
        index += 1
        if index >= len(
            file_list
        ):  # If 'n' is pressed at the last image, exit the program
            break
        final_image = process_image(index, threshold_value)
        cv2.imshow("Image", final_image)
    elif key == ord(","):  # If 'p' is pressed, move to the previous image
        index -= 1
        if index < 0:  # If 'p' is pressed at the first image, exit the program
            break
        final_image = process_image(index, threshold_value)
        cv2.imshow("Image", final_image)
